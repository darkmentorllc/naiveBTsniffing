...
{
	DBusMessageIter iter;
	const char *address, *name, *addressType;//, *msdCID;
	char type = 0; // 0 for LE, 1 for BTC
//	unsigned short msdCID;
//	struct eir_msd *msd;
	...
	// There is apparently no good way to simply detect a device is BLE vs. BTC after the fact if you're scanning for both
	// (And it turns out that if I run two separate instances of bluetoothctl, one scanning for "le", and one for "bredr"
	// the results effectively become the same as "scan on" which scans for both)
	// So use the heuristic that if something has a CoD it's almost certainly BTC (there are some BLE devices that advertise
	// a CoD but it's very rare according to my data (170 LE vs. 11950 for BTC, so I'm willing to accept that false positive rate)
	if (g_dbus_proxy_get_property(proxy, "Class", &iter) == TRUE){
		type = 1;
	}

/*
	// If the device has Manufacturer-Specific Data (MSD) then I want to get the company ID (CID) out of that,
	// that way I can deprioritize Apple devices later (at the cost of false positives due to iBeacons)
	if (g_dbus_proxy_get_property(proxy, "ManufacturerData", &iter) == TRUE){
		dbus_message_iter_get_basic(&iter, &msd);
//		bt_shell_printf("Got MSD CID of %x\n", msd->company);
	}
	else {
		msdCID = "NA";
	}
*/
	...
	bt_shell_printf("%s%s%sDevice %s %s %s %s\n",
				description ? "[" : "",
				description ? : "",
				description ? "] " : "",
				address,
				(strcmp(addressType, "public") == 0 && type == 1) ? "BTC" : "BLE",
				addressType,
				name);
}
