...
def process_sequence(sequence_element, indent):
    ...
    for seq_item in sequence_element:
        if seq_item.tag == 'uint16' or seq_item.tag == 'uuid' or seq_item.tag == 'uint8':
            ...
            item_type = seq_item.tag
            print(f"{indentation}{item_type} {value}")
        elif seq_item.tag == 'sequence':
            # Increase the indent level for nested sequences
            process_sequence(seq_item, indent + 2)
