...
def process_element(element, indent=0):
    ...
    for child in element:
        if child.tag == 'sequence':
            ...
            for seq_elem in child:
                seq_value_elem = seq_elem.find('*[@value]')
                if seq_value_elem is not None:
                    seq_value = seq_value_elem.get('value')
                    sequence_values.append(seq_value)

            if sequence_values:
                print(f"{indentation}  Sequence Values: {sequence_values}")
