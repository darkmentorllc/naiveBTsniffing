...
static char * g_log_name = "/home/pi/SDPprint.log";
FILE * g_log_FILE;
	...
	case SDP_INT128:
		if (member_name) {
			...
			printf("%.*sInteger : 0x%x\n", indent, indent_spaces,
				sdpdata->val.uint32);
//			printf("SDPPRINT:Integer,0x%x\n",sdpdata->val.uint32);
			fprintf(g_log_FILE, "SDPPRINT:Integer,0x%x\n",sdpdata->val.uint32);
		}
		break;
...
//		printf("SDPPRINT:AttributeIdentifier:0x%x,%s\n", attrId, attrDef->name);
		fprintf(g_log_FILE,"SDPPRINT:AttributeIdentifier:0x%x,%s\n", attrId, attrDef->name);
	}
	else
...
//		printf("SDPPRINT:Service Class ID List:\n");
		fprintf(g_log_FILE, "SDPPRINT:Service Class ID List:\n");
		sdp_list_foreach(list, print_service_class, 0);
		sdp_list_free(list, free);
	}
	if (sdp_get_access_protos(rec, &proto) == 0) {
		printf("Protocol Descriptor List:\n");
		fprintf(g_log_FILE, "Protocol Descriptor List:\n");
	...
	if (sdp_get_lang_attr(rec, &list) == 0) {
		...
		fprintf(g_log_FILE, "Language Base Attr List:\n");
		sdp_list_foreach(list, print_lang_attr, 0);
		sdp_list_free(list, free);
	}
	if (sdp_get_profile_descs(rec, &list) == 0) {
		printf("Profile Descriptor List:\n");
		fprintf(g_log_FILE, "Profile Descriptor List:\n");
